...
This document explains how WhatsApp detects, enforces, and handles account bans and restrictions — extracted from the WhatsApp APK source code. Understanding these mechanisms helps you write bots that avoid triggering enforcement and handle enforcement events correctly.

---

## Overview

WhatsApp has several distinct enforcement layers that operate independently:
...
Bans are surfaced in HTTP responses from WhatsApp's registration and session endpoints. The client parses these fields from the JSON response body:
...
Gap enforcement evaluates content rules against "viewport snapshots" of the user's inbox. It enforces visual/UX rules (e.g. certain chats cannot appear in the pinned inbox area, or certain content types cannot appear in the top of the archived inbox).
...
Gap enforcement is primarily a **client-side rendering enforcement** — it controls what appears where in the UI, not whether messages are delivered.

---

## Newsletter / Channel Enforcement

Newsletter (WhatsApp Channels) has a dedicated enforcement system through `NewsletterEnforcementsClient`.
...
These are the baron-baileys-v2 methods that interact with the enforcement systems:
...
Based on the violation types and enforcement code, these behaviors are associated with enforcement:
...
The enforcement reporting methods above let you **respond** to enforcement actions that the server reports — use them to build logic around detecting and recovering from bans in long-running bots.
